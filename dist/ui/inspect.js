...
import { formatDuration, truncateVisible, visibleWidth } from './ascii.js';
function clean(text) {
...
function footer(state, scroll, total, theme, width) {
    ...
    return truncateVisible(theme.muted(text) + (state.status === 'RUNNING' ? theme.accent('   (live)') : ''), width);
}
function fit(lines, viewport, scroll) {
...
export function renderGoalView(state, theme, viewport, scroll) {
    const width = Math.max(20, viewport.width);
    const contentWidth = Math.max(16, width - 4);
    const lines = [
        theme.secondary(`Runtime: ${state.runtimeId}   Model: ${state.model ?? 'runtime default'}   Status: ${state.status}`),
        theme.secondary(`Phase: ${state.currentRoleId ?? 'complete'}   Fix cycles: ${state.attempts}   Events: ${state.events.length}`),
        '',
...
export function renderActivityView(state, theme, viewport, scroll) {
    const width = Math.max(20, viewport.width);
    const contentWidth = Math.max(16, width - 4);
    const lines = [];
    state.events.forEach((event, index) => {
        ...
        for (const continuation of rest)
            for (const wrapped of wrapText(continuation, contentWidth - 6))
                lines.push(`      ${wrapped}`);
    });
    if (lines.length === 0)
        lines.push(theme.muted('No events yet.'));
    const fitted = fit(lines, viewport, scroll);
    return {
        total: fitted.total,
        lines: [
...
export function renderAgentView(state, roleId, theme, viewport, scroll) {
    const width = Math.max(20, viewport.width);
    const contentWidth = Math.max(16, width - 4);
    ...
    const lines = [
        ...
        theme.secondary(`Budget: ${role ? `${role.budget.maxSteps} steps / ${role.budget.maxToolCalls} tool calls` : 'n/a'}   ` +
    ...
    events.forEach((event, index) => {
        ...
        for (const wrapped of wrapText(event.message, contentWidth - 4))
            lines.push(`    ${wrapped}`);
    });
    if (events.length === 0)
        lines.push(theme.muted('  No events yet for this role.'));
...
export function renderHelpView(theme, viewport) {
    const width = Math.max(20, viewport.width);
    const lines = [
        theme.accent('Mouse'),
