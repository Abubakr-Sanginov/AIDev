...
const SECRET_FILE = /(^|\/)(\.env(?:\..*)?|.*(?:credentials?|secrets?|private[-_.]?key).*|id_(?:rsa|dsa|ecdsa|ed25519)(?:\.pub)?|.*\.(?:pem|p12|pfx|key|keystore))$/i;
const MANIFEST = /(^|\/)(package\.json|pyproject\.toml|requirements(?:-[^/]+)?\.txt|pom\.xml|build\.gradle(?:\.kts)?|go\.mod|cargo\.toml|composer\.json|gemfile|mix\.exs|pubspec\.yaml)$/i;
const LOCKFILE = /(^|\/)(package-lock\.json|npm-shrinkwrap\.json|yarn\.lock|pnpm-lock\.yaml|bun\.lockb?|poetry\.lock|pipfile\.lock|cargo\.lock|composer\.lock|gemfile\.lock)$/i;
const BUILD_CONFIG = /(^|\/)(tsconfig(?:\.[^/]+)?\.json|vite\.config\.[^/]+|webpack\.[^/]+|rollup\.config\.[^/]+|next\.config\.[^/]+|nuxt\.config\.[^/]+|makefile|cmakelists\.txt|dockerfile|compose\.ya?ml)$/i;
const TEST_CONFIG = /(^|\/)(vitest\.config\.[^/]+|jest\.config\.[^/]+|playwright\.config\.[^/]+|cypress\.config\.[^/]+|pytest\.ini|tox\.ini|phpunit\.xml|.*\.test\.[^/]+|.*\.spec\.[^/]+)$/i;
const LINT_CONFIG = /(^|\/)(eslint\.config\.[^/]+|\.eslintrc(?:\.[^/]+)?|biome\.jsonc?|ruff\.toml|\.ruff\.toml|pylintrc|golangci\.ya?ml|checkstyle\.xml)$/i;
const FORMAT_CONFIG = /(^|\/)(\.prettierrc(?:\.[^/]+)?|prettier\.config\.[^/]+|\.editorconfig|biome\.jsonc?|rustfmt\.toml|\.clang-format)$/i;
const INSTRUCTIONS = /(^|\/)(agents\.md|claude\.md|copilot-instructions\.md|contributing\.md|readme\.md|development\.md|docs\/[^/]*(?:contribut|develop|architect|style)[^/]*\.md)$/i;
...
export async function validateProjectRoot(root) {
...
export async function inspectProject(root, options = {}) {
    ...
    while (queue.length > 0) {
        ...
        for (const entry of entries) {
            ...
            inspectedEntries += 1;
            const relative = (current.relative ? `${current.relative}/${entry.name}` : entry.name).replaceAll('\\', '/');
            ...
            else if (entry.isFile() && !SECRET_FILE.test(relative))
                files.push(relative);
        }
    }
    ...
    const lockfiles = sorted(files.filter((file) => LOCKFILE.test(file)), maxPaths);
    const packageManagerField = typeof packageData.packageManager === 'string' ? packageData.packageManager : undefined;
    ...
    const frontend = [...dependencies].some((name) => [
        ...
        names.some((name) => /^(src\/)?(app|pages|components|views|client|frontend)\//.test(name) ||
            /\.(tsx|jsx|vue|svelte)$/.test(name));
    const backend = [...dependencies].some((name) => [
        'express',
        'fastify',
        '@nestjs/core',
        'koa',
        ...
        names.some((name) => /^(src\/)?(api|server|backend|routes|controllers|services|database|db)\//.test(name) ||
    ...
    return {
        ...
        truncated,
        inspectedEntries,
        manifests: sorted(files.filter((file) => MANIFEST.test(file)), maxPaths),
        scripts: sorted(Object.keys(packageData.scripts ?? {}), maxPaths),
        ...(packageManager === undefined ? {} : { packageManager }),
        lockfiles,
        languages,
        buildConfigs: sorted(files.filter((file) => BUILD_CONFIG.test(file)), maxPaths),
        testConfigs: sorted(files.filter((file) => TEST_CONFIG.test(file)), maxPaths),
        lintConfigs: sorted(files.filter((file) => LINT_CONFIG.test(file)), maxPaths),
        formatConfigs: sorted(files.filter((file) => FORMAT_CONFIG.test(file)), maxPaths),
        instructionDocs: sorted(files.filter((file) => INSTRUCTIONS.test(file)), maxPaths),
        topLevelTree: sorted([...directories, ...files].filter((name) => name.split('/').length <= 2), maxPaths),
        representativeSourcePaths: sorted(files.filter((file) => SOURCE_PATH.test(file) && !TEST_PATH.test(file)), maxPaths),
        representativeTestPaths: sorted(files.filter((file) => TEST_PATH.test(file)), maxPaths),
        layers: { frontend, backend },
    };
}
export function formatProjectContext(context, maxCharacters = 12_000) {
