 ...
 */
export class ApiProviderRuntime {
    id;
    name;
    #provider;
    #root;
    ...
    async execute(session, request) {
        ...
        try {
            ...
            const runTool = async (call) => {
                ...
                if (result.ok && seenCalls.size < 200)
            ...
            if (this.#provider.protocol === 'openai') {
                ...
                for (let step = 0; step < hardSteps; step++) {
                    ...
                    if (step === hardSteps - 1)
                        throw new Error(`Safety ceiling reached after ${hardSteps} steps without a final answer.`);
                }
            }
            else {
                ...
                for (let step = 0; step < hardSteps; step++) {
                    const { reply, assistantContent } = await callAnthropicMessages({
                        ...
                        apiKey: resolution.key,
                        model,
                        system: 'You are an autonomous software-engineering agent. Use the provided tools when you need to inspect or modify the project.',
                        messages,
                        tools: schemas,
                        ...(this.#fetchImpl === undefined ? {} : { fetchImpl: this.#fetchImpl }),
                        ...(this.#transportDelaysMs === undefined ? {} : { transportDelaysMs: this.#transportDelaysMs }),
