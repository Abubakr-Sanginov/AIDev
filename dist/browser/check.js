...
import { detectWebTarget, findLocalUrl, formatBrowserFailure, formatBrowserReport, } from './target.js';
...
async function startScriptServer(root, command, extraEnv, timeoutMs) {
...
async function startServer(target, root, timeoutMs) {
...
async function explore(page, url, screenshotDir, root, maxClicks, say) {
    ...
    const addDefect = (defect) => {
        if (defects.size < 25)
            defects.add(defect.slice(0, 300));
    };
    ...
    if (!hasContent)
        addDefect('The page rendered nothing visible (blank page).');
    await shoot('01-top');
    ...
    for (let step = 0; step < 30; step += 1) {
        ...
        screens += 1;
        if (screens === 3)
            await shoot('02-middle');
    }
    await shoot('03-bottom');
    await page.evaluate(() => window.scrollTo(0, 0));
    say('Browser check: clicking interactive elements');
    const clickables = page.locator('a[href], button, [role="button"], summary, input[type="button"], input[type="submit"]');
    ...
    for (let index = 0; index < interactive && clicked < maxClicks; index += 1) {
        ...
        if (href !== null) {
            if (/^(?:mailto:|tel:|javascript:)/i.test(href))
                continue;
            try {
                if (new URL(href, page.url()).origin !== origin)
                    continue;
            }
            catch {
                continue;
        ...
        const label = await element
            .evaluate((node) => (node.textContent.trim() || node.getAttribute('aria-label') || node.tagName).slice(0, 40))
            .catch(() => 'element');
        ...
        try {
            ...
            notes.push(`Could not click "${label}": ${(error instanceof Error ? error.message : String(error)).split('\n')[0] ?? ''}`);
            continue;
        }
    ...
    let typed = 0;
    const inputs = page.locator('input[type="text"], input[type="search"], input:not([type]), textarea, [contenteditable="true"]');
...
export async function runBrowserCheck(options) {
    ...
    if (target.kind === 'script' && target.needsInstall)
        return {
            status: 'fail',
            report: formatBrowserFailure(`The site cannot start: dependencies are not installed (no node_modules). Run \`${target.packageManager} install\`.`),
        };
    say(target.kind === 'static'
    ...
    try {
        ...
        return {
            status: 'fail',
            report: formatBrowserFailure(`The site did not start: ${error instanceof Error ? error.message : String(error)}`),
        };
    }
    let launched;
    try {
        ...
        try {
            const findings = await explore(page, server.url, screenshotDir, options.root, options.maxClicks ?? 15, say);
