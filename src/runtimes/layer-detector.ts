...
async function packageSignals(root: string): Promise<ProjectLayers> {
  try {
    ...
    return {
      ...
      backend:
        [...dependencies].some((name) => BACKEND_DEPENDENCIES.has(name)) ||
...
async function projectPaths(root: string, limit = 500): Promise<string[]> {
  const queue = [''];
  const found: string[] = [];
...
export async function detectProjectLayers(root: string): Promise<ProjectLayers> {
  ...
  const frontend =
    ...
    names.some(
      (name) =>
        /^(app|pages|components|public|styles|web|client|frontend)\//i.test(name) ||
  ...
  const backend =
    ...
    names.some(
      (name) =>
        /^(api|server|backend|routes|controllers|models|services|database|db)\//i.test(name) ||
        /^src\/(api|server|routes|controllers|models|services)\//i.test(name) ||
        /(^|\/)(compose\.ya?ml|pyproject\.toml|requirements\.txt|pom\.xml|build\.gradle|go\.mod|cargo\.toml)$/i.test(
          name,
        ),
    );
