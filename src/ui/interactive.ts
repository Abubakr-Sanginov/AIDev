import { hitTest, type Rect } from './ascii.js';

export type MouseButton = 'left' | 'wheel-up' | 'wheel-down' | 'other';

export type TerminalEvent =
  | { type: 'mouse'; button: MouseButton; x: number; y: number }
  | { type: 'key'; key: string };
 ...
 */
export function parseTerminalInput(chunk: string): TerminalEvent[] {
  ...
  if (consumedUntil < chunk.length) events.push(...parseKeys(chunk.slice(consumedUntil)));
  return events;
}

function keyForSequence(params: string, final: string): string | undefined {
  if (final === 'A') return 'arrow-up';
  if (final === 'B') return 'arrow-down';
  if (final === 'C') return 'arrow-right';
  if (final === 'D') return 'arrow-left';
  if (final === 'H') return 'home';
  if (final === 'F') return 'end';
  if (final !== '~') return undefined;
  if (params === '5') return 'page-up';
  if (params === '6') return 'page-down';
...
export const INITIAL_UI_STATE: UiState = { view: { kind: 'dashboard' }, scroll: 0, hotspots: [] };

const AGENT_KEYS: Readonly<Record<string, string>> = {
  '1': 'manager',
  '2': 'architect',
  '3': 'backend',
  '4': 'frontend',
...
function clampScroll(scroll: number): number {
  return Math.max(0, Math.round(scroll));
}

function openView(state: UiState, view: ViewKind): UiState {
  return { view, scroll: 0, hotspots: state.hotspots };
}

/** Pure reducer: maps one terminal event to the next UI state. */
export function reduceUiEvent(state: UiState, event: TerminalEvent, pageLines: number): UiState {
  const page = Math.max(1, pageLines);
  if (event.type === 'mouse') {
    if (state.view.kind === 'dashboard') {
      if (event.button !== 'left') return state;
      const target = hitTest(state.hotspots, event.x, event.y);
      if (target === undefined) return state;
      if (target === 'overview') return openView(state, { kind: 'goal' });
      if (target === 'activity') return openView(state, { kind: 'activity' });
      if (target.startsWith('agent:'))
        return openView(state, { kind: 'agent', roleId: target.slice('agent:'.length) });
      return state;
    }
    if (event.button === 'left') return openView(state, { kind: 'dashboard' });
  ...
  if (key === 'g') return openView(state, { kind: 'goal' });
  if (key === 'a') return openView(state, { kind: 'activity' });
  if (key === 'h' || key === '?') return openView(state, { kind: 'help' });
  if (state.view.kind === 'dashboard') {
    const roleId = AGENT_KEYS[key];
    return roleId === undefined ? state : openView(state, { kind: 'agent', roleId });
  }
  if (key === 'arrow-up') return { ...state, scroll: clampScroll(state.scroll + 1) };
  if (key === 'arrow-down') return { ...state, scroll: clampScroll(state.scroll - 1) };
  if (key === 'page-up') return { ...state, scroll: clampScroll(state.scroll + page) };
  if (key === 'page-down') return { ...state, scroll: clampScroll(state.scroll - page) };
  if (key === 'home') return { ...state, scroll: Number.MAX_SAFE_INTEGER };
  if (key === 'end') return { ...state, scroll: 0 };
  return state;
}

function parseKeys(input: string): TerminalEvent[] {
  const events: TerminalEvent[] = [];
  let index = 0;
  while (index < input.length) {
    if (input.startsWith(`${ESC}[`, index)) {
      ...
      if (sequence) {
        ...
        if (key !== undefined) events.push({ type: 'key', key });
    ...
    if (char >= ' ') events.push({ type: 'key', key: char });
    index += 1;
  }
  return events;
}
