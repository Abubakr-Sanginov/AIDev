import type { LLMProvider, ProjectState, Role } from './types.js';
...
export class Orchestrator {
  ...
  async run(goal: string): Promise<ProjectState> {
    ...
    const manager = await this.#runRole(
      'Manager',
      state,
      `Plan this task and define acceptance criteria:\n${goal}`,
    );
    const architect = await this.#runRole(
      'Architect',
      state,
      `Design the solution for this goal and write .ai-team/architecture.md:\n${goal}`,
      manager,
    );
    await this.#runRole(
      'Coder',
      state,
      `Implement the goal using the manager plan and architecture. Complete actual file and command work.`,
      `${manager}\n\n${architect}`,
    );

    let report = await this.#runRole(
      'Tester',
      state,
      'Run all applicable verification. Do not modify source. Give exact failures and a verdict.',
    );
    for (let attempt = 0; !this.#passed(report) && attempt < this.#maxFixAttempts; attempt++) {
      ...
      const fix = await this.#runRole(
        'Fixer',
        state,
        'Fix every root cause in the tester report.',
        report,
      );
      report = await this.#runRole(
        'Tester',
        state,
        'Re-run all applicable verification after fixes and give a verdict.',
        fix,
      );
    }
    ...
    const review = await this.#runRole(
      'Reviewer',
      state,
      `Review the completed project against this goal:\n${goal}`,
      report,
    );
    if (!this.#passed(review)) {
      const fix = await this.#runRole(
        'Fixer',
        state,
        'Fix the actionable reviewer findings.',
        review,
      );
      report = await this.#runRole(
        'Tester',
        state,
        'Verify the reviewer fixes and give a verdict.',
        fix,
      );
      if (!this.#passed(report)) return this.#fail(state, 'Review fixes failed verification.');
    }
    state.status = 'DONE';
  ...
  async #runRole(role: Role, state: ProjectState, task: string, handoff = ''): Promise<string> {
    const item = state.tasks.find((candidate) => candidate.role === role);
    if (!item) throw new Error(`Task for ${role} was not initialized.`);
    item.status = 'RUNNING';
  ...
  #task(id: string, role: Role, description: string) {
    return { id, role, description, status: 'WAITING' as const };
  }
  async #persist(state: ProjectState): Promise<void> {
    await this.#memory.saveState(state);
    await this.#memory.savePlan(state.tasks);
    this.#ui.render(state, this.#activity);
  }
  async #fail(state: ProjectState, reason: string): Promise<ProjectState> {
    state.status = 'FAILED';
