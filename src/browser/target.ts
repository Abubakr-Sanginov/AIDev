...
async function detectPackageManager(
  ...
  for (const candidate of ['pnpm', 'yarn', 'bun', 'npm'] as const)
    if (declared.startsWith(`${candidate}@`)) return candidate;
...
export async function detectWebTarget(root: string): Promise<WebTarget | undefined> {
...
export function findLocalUrl(output: string): string | undefined {
 ...
 */
export function formatBrowserReport(findings: BrowserFindings): string {
  const lines = [
    '## Browser check',
  ...
  if (findings.defects.length > 0) {
    ...
    for (const defect of findings.defects) lines.push(`- ${defect}`);
  }
  if (findings.notes.length > 0) {
    lines.push('', 'Notes:');
    for (const note of findings.notes) lines.push(`- ${note}`);
  }
  if (findings.screenshots.length > 0)
    lines.push('', `Screenshots: ${findings.screenshots.join(', ')}`);
  lines.push(
    '',
    findings.defects.length > 0 ? 'VERDICT: FAIL (browser check)' : 'Browser check: PASS',
  );
  return lines.join('\n');
}

/** A failure before any page could be inspected (no install, server crash). */
export function formatBrowserFailure(reason: string): string {
